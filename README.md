...
## Изменения, по сравнению с форком:

1. Логика туннеля вынесена в общий пакет `tunnel/` — CLI-клиент стал тонкой обёрткой, код переиспользуется Android-клиентом ([vk-turn-proxy-android](https://github.com/Katze-942/vk-turn-proxy-android))
2. IPv4-only режим (`-4`) передаёт custom dialer в `getVkCreds`/`getYandexCreds` вместо глобальных переменных
